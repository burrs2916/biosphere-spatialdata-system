...
const DIR: &str = "/Users/liwenchao/EdgeView/biosphere/biosphere-spatialdata-system/docs/需求分析/20260510";
...
fn find_clusters(sorted: &[f64], num_buckets: usize, max_gap_buckets: usize) -> Vec<(f64, f64, usize)> {
    if sorted.is_empty() {
        return vec![];
    }
    let n = sorted.len();
    ...
    let mut result: Vec<(f64, f64, usize)> = clusters
        ...
        .map(|(s, e, c)| {
            ...
            let precise_min = sorted.iter().cloned().find(|&v| v >= min_v).unwrap_or(global_min);
            let precise_max = sorted.iter().cloned().rev().find(|&v| v <= max_v).unwrap_or(global_max);
...
fn analyze_file(path: &std::path::Path) {
    ...
    if has_large_coords { problem_count += 1; }
    if has_heavy_lwpoly { problem_count += 1; }
    if has_heavy_hatch { problem_count += 1; }
    if has_extreme_text { problem_count += 1; }
    ...
    let current_profile = if entity_count == 0 {
        ...
        if has_large_coords { "LargeCoordinates".to_string() }
        else if has_heavy_lwpoly { "HeavyLwPolyline".to_string() }
        else if has_heavy_hatch { "HeavyHatch".to_string() }
        else { "Complex(ExtText)".to_string() }
    } else if problem_count >= 2 {
        format!("Complex(flags={:04b})", 
    ...
    let new_profile = if entity_count == 0 {
        ...
        if has_large_coords_new { flags.push_str("LC "); }
        if has_heavy_lwpoly { flags.push_str("LP "); }
        if has_heavy_hatch { flags.push_str("HH "); }
        if has_extreme_text { flags.push_str("ET "); }
        if has_distributed_coords_new { flags.push_str("DC "); }
        if has_medium_entity { flags.push_str("ME "); }
    ...
    if !x_clusters.is_empty() {
        ...
        if x_clusters.len() >= 2 {
            ...
            let gap_abs = gap.abs();
            let max_span = (x_clusters[0].1 - x_clusters[0].0).max(x_clusters[1].1 - x_clusters[1].0);
            let gap_ratio = if max_span > 0.0 { gap_abs / max_span } else { 0.0 };
        }
    }
    if !y_clusters.is_empty() {
        ...
        if y_clusters.len() >= 2 {
            ...
            let gap_abs = gap.abs();
            let max_span = (y_clusters[0].1 - y_clusters[0].0).max(y_clusters[1].1 - y_clusters[1].0);
            let gap_ratio = if max_span > 0.0 { gap_abs / max_span } else { 0.0 };
        }
    }
    if !all_xs_sorted.is_empty() && !all_ys_sorted.is_empty() {
        let (main_x_min, main_x_max, _) = if !x_clusters.is_empty() {
            ...
            (all_xs_sorted[0], all_xs_sorted[all_xs_sorted.len() - 1], all_xs_sorted.len())
        };
        let (main_y_min, main_y_max, _) = if !y_clusters.is_empty() {
            (y_clusters[0].0, y_clusters[0].1, y_clusters[0].2)
        } else {
            (all_ys_sorted[0], all_ys_sorted[all_ys_sorted.len() - 1], all_ys_sorted.len())
        };
    ...
    if entity_count >= 10000 && entity_count < 30000 {
        risks.push(format!("中等实体数({})→当前Simple,建议MediumEntity", entity_count));
    }
    ...
    if x_clusters.len() > 1 || y_clusters.len() > 1 {
        risks.push(format!("多集群分布(X={},Y={})", x_clusters.len(), y_clusters.len()));
    }
    if lwpoly_huge > 0 {
        risks.push(format!("超大多段线({}条>1000顶点, 最大{}顶点)", lwpoly_huge, lwpoly_max_verts));
    }
    if hatch_count > 2000 || hatch_max_edges > 100 {
        risks.push(format!("复杂填充(count={}, max_edges={})", hatch_count, hatch_max_edges));
    }
    ...
    if !all_xs.is_empty() {
        let outlier_ratio = {
            let (main_x_min, main_x_max, _) = x_clusters.first().copied().unwrap_or((min_x, max_x, all_xs.len()));
            let (main_y_min, main_y_max, _) = y_clusters.first().copied().unwrap_or((min_y, max_y, all_ys.len()));
            let mx = (main_x_max - main_x_min).max(200.0);
            let my = (main_y_max - main_y_min).max(200.0);
            let out_count = all_xs.iter().zip(all_ys.iter())
                .filter(|(&x, &y)| x < main_x_min - mx || x > main_x_max + mx || y < main_y_min - my || y > main_y_max + my)
                .count();
