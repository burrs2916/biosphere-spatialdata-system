...
fn find_all_clusters(sorted_vals: &[f64], max_gap_buckets: usize) -> Vec<CoordCluster> {
    ...
    if let Some(start) = seg_start {
        if seg_count > 0 {
            let c_min = global_min + start as f64 * bucket_width;
            let precise_min = sorted_vals.iter().cloned().find(|&v| v >= c_min).unwrap_or(global_min);
            clusters.push(CoordCluster { min: precise_min, max: global_max, count: seg_count });
        }
    }
...
fn find_significant_cluster_range_v2(sorted_vals: &[f64]) -> (f64, f64) {
    if sorted_vals.is_empty() { return (0.0, 0.0); }
...
fn analyze(path: &str, name: &str) {
    ...
    for entity in doc.entities() {
        ...
        match &entity {
            EntityType::Line(l) => {
                if l.start.x.is_finite() { xs.push(l.start.x); ys.push(l.start.y); }
                if l.end.x.is_finite() { xs.push(l.end.x); ys.push(l.end.y); }
            }
            EntityType::Circle(c) => { if c.center.x.is_finite() { xs.push(c.center.x); ys.push(c.center.y); } }
            EntityType::Insert(ins) => { if ins.insert_point.x.is_finite() { xs.push(ins.insert_point.x); ys.push(ins.insert_point.y); } }
            EntityType::LwPolyline(lw) => {
                for v in lw.vertices.iter().take(5) {
                    if v.location.x.is_finite() { xs.push(v.location.x); ys.push(v.location.y); }
                }
            }
            EntityType::MText(mt) => { if mt.insertion_point.x.is_finite() { xs.push(mt.insertion_point.x); ys.push(mt.insertion_point.y); } }
            EntityType::Text(t) => { if t.insertion_point.x.is_finite() { xs.push(t.insertion_point.x); ys.push(t.insertion_point.y); } }
            _ => {}
        }
        if xs.len() >= 10000 { break; }
    }
    ...
    // 计算偏移
    let filtered_xs: Vec<f64> = xs.iter().cloned().filter(|&x| x >= xmin - (span_x*0.05).max(200.0) && x <= xmax + (span_x*0.05).max(200.0)).collect();
    let filtered_ys: Vec<f64> = ys.iter().cloned().filter(|&y| y >= ymin - (span_y*0.05).max(200.0) && y <= ymax + (span_y*0.05).max(200.0)).collect();
