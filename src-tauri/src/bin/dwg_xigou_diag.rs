...
fn main() {
    ...
    for entity in doc.entities() {
        match &entity {
            ...
            EntityType::MText(mt) => {
                text_details.push(("MText".to_string(), mt.insertion_point.x, mt.insertion_point.y, mt.height));
                y_values.push(("MText".to_string(), mt.insertion_point.y, mt.insertion_point.y));
            }
            EntityType::Text(t) => {
                text_details.push(("Text".to_string(), t.insertion_point.x, t.insertion_point.y, t.height));
                y_values.push(("Text".to_string(), t.insertion_point.x, t.insertion_point.y));
            }
            EntityType::Insert(ins) => {
                insert_details.push((ins.block_name.clone(), ins.insert_point.x, ins.insert_point.y, ins.x_scale(), ins.y_scale(), ins.rotation));
                y_values.push(("Insert".to_string(), ins.insert_point.y, ins.insert_point.y));
            }
            EntityType::Circle(c) => {
                y_values.push(("Circle".to_string(), c.center.y - c.radius, c.center.y + c.radius));
            }
            EntityType::Arc(a) => {
                y_values.push(("Arc".to_string(), a.center.y - a.radius, a.center.y + a.radius));
            }
    ...
    let mut all_y: Vec<f64> = y_values.iter()
    ...
    if !all_y.is_empty() {
        ...
        let between = all_y.iter().filter(|&&y| y >= -1000.0 && y <= 5000.0).count();
    }
    
    let mut by_block: HashMap<String, Vec<(f64, f64, f64, f64, f64)>> = HashMap::new();
    for (name, x, y, sx, sy, rot) in &insert_details {
        by_block.entry(name.clone()).or_default().push((*x, *y, *sx, *sy, *rot));
    }
    
    let mut block_stats: Vec<_> = by_block.iter().map(|(name, items)| {
    ...
    for (_, _, y, h) in &text_details {
        let range = if *y < -1000.0 { "Y < -1000" }
        else if *y < 0.0 { "-1000 <= Y < 0" }
        else if *y < 1000.0 { "0 <= Y < 1000" }
        else if *y < 3000.0 { "1000 <= Y < 3000" }
        else if *y < 5000.0 { "3000 <= Y < 5000" }
        else { "Y >= 5000" };
    ...
    let small_texts: Vec<_> = text_details.iter().filter(|(_, _, _, h)| *h < 1.0).collect();
        ...
        match &entity {
            EntityType::Line(l) => {
                if l.start.y < -1000.0 || l.end.y < -1000.0 {
                    negative_entities.push(format!("Line ({:.1},{:.1}) -> ({:.1},{:.1})", l.start.x, l.start.y, l.end.x, l.end.y));
                }
            }
            EntityType::MText(mt) => {
                if mt.insertion_point.y < -1000.0 {
                    negative_entities.push(format!("MText ({:.1},{:.1}) h={:.2} value={:.30}...", mt.insertion_point.x, mt.insertion_point.y, mt.height, mt.value));
                }
            }
            EntityType::Insert(ins) => {
                if ins.insert_point.y < -1000.0 {
                    negative_entities.push(format!("Insert {} ({:.1},{:.1})", ins.block_name, ins.insert_point.x, ins.insert_point.y));
                }
            }
            _ => {}
        }
        ...
        match &entity {
            EntityType::Line(l) => {
                if l.start.y > 5000.0 || l.end.y > 5000.0 {
                    high_entities.push(format!("Line ({:.1},{:.1}) -> ({:.1},{:.1})", l.start.x, l.start.y, l.end.x, l.end.y));
                }
            }
            EntityType::MText(mt) => {
                if mt.insertion_point.y > 5000.0 {
                    high_entities.push(format!("MText ({:.1},{:.1}) h={:.2} value={:.30}...", mt.insertion_point.x, mt.insertion_point.y, mt.height, mt.value));
                }
            }
            EntityType::Insert(ins) => {
                if ins.insert_point.y > 5000.0 {
                    high_entities.push(format!("Insert {} ({:.1},{:.1})", ins.block_name, ins.insert_point.x, ins.insert_point.y));
                }
            }
            _ => {}
        }
